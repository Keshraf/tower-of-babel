// content/components/TooltipPortal.tsx (React UI)
import { useState, useEffect } from "react";
import { speakFrench } from "../content/utils/tts";
...
export default function TooltipPortal({ state }: any) {
  const [wordData, setWordData] = useState<WordData | null>(null);
  const [position, setPosition] = useState({ x: 0, y: 0 });

  useEffect(() => {
    const update = () => {
      setWordData(state.currentWord);
